...
def show_help
  puts "Usage: ruby parse_amendments.rb [OPTIONS] [MESSAGE_TYPE] [FILE_PATH_PATTERN]"
  puts
  puts "Description:"
  ...
  puts "  --overwrite, -o       Overwrite existing files instead of creating new ones with numeric suffixes"
  ...
  puts "  ruby parse_amendments.rb -t general running_annexes  # Parse all running_annexes general messages"
  puts "  ruby parse_amendments.rb -t general approved_recommendations  # Parse all approved_recommendations general messages"
  puts "  ruby parse_amendments.rb -o -t general iptn        # Parse all IPTN general messages, overwriting existing files"
  puts
end

# Parse command line arguments
args = ARGV.dup
options = {type: 'amendment', overwrite: false}
...
opt_parser = OptionParser.new do |opts|
  ...
  opts.on("-o", "--overwrite", "Overwrite existing files instead of creating new ones with numeric suffixes") do
...
issue_num = filename.gsub(/\.yaml$/, '').scan(/\d+/).first || "unknown"

# Create output directory if it doesn't exist
output_dir = File.join(__dir__, "../actions/#{options[:type] == 'amendment' ? 'amendments' : 'general'}/#{message_type}")
FileUtils.mkdir_p(output_dir)

# Check for manual files for any message type
manual_file = File.join(base_dir, message_type, 'manual', filename)
if File.exist?(manual_file)
  puts "Using manually curated file: #{manual_file}"
  yaml_data = YAML.load_file(manual_file)
  yaml_data['manual'] = true
  message = yaml_data # Already in correct format
else
  ...
  # Call any post-processing methods if they exist
  if message.respond_to?(:extract_trial_info_from_notes)
    message.extract_trial_info_from_notes
  end
end

# Check if we've already processed this file in this run
@processed_files ||= {}
file_key = "#{options[:type]}/#{message_type}/#{filename}"

if @processed_files[file_key]
  # Skip this file as we've already processed it in this run
  puts "Skipping duplicate processing of #{filename}"
  next
end

@processed_files[file_key] = true

# Handle multiple entries with the same issue number
# Only check for existing files if we're not overwriting them
if !options[:overwrite] && ENV['OVERWRITE_FILES'] != 'true'
  existing_files = Dir.glob(File.join(output_dir, "#{issue_num}.yaml")) +
                   Dir.glob(File.join(output_dir, "#{issue_num}-*.yaml"))

  if existing_files.empty?
    # First file for this issue number
    output_file = File.join(output_dir, "#{issue_num}.yaml")
  else
    # Add numeric prefix for additional files
    max_number = existing_files.map { |f| f.scan(/-(\d+)\.yaml$/).flatten.first.to_i }.max || 0
    output_file = File.join(output_dir, "#{issue_num}-#{max_number + 1}.yaml")
  end
else
  # Overwrite mode - use the base filename
  output_file = File.join(output_dir, "#{issue_num}.yaml")
end

File.write(output_file, message.to_yaml)
puts "Saved parsed message to #{output_file}"

    # Print the parsed content
    puts message.to_yaml

    # Print additional information for specific message types
    if options[:type] == 'general' && message_type == 'running_annexes' && message.is_a?(Ituob::Models::GeneralRunningAnnexes)
      puts
      puts "== Running Annexes Details =="
      puts "Type: #{message.type}"
      if message.respond_to?(:extra_links) && message.extra_links && !message.extra_links.empty?
        puts "Extra Links (#{message.extra_links.size}):"
        message.extra_links.each_with_index do |link, index|
          puts "  #{index + 1}. #{link}"
        end
      else
        puts "No extra links found."
      end
    end
