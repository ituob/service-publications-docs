...
* Extend the `parse_amendments.rb` script to parse all messages in the
  `messages/` directory.
...
amendment types from extracted amendment files or general message types from
extracted general message files. This is particularly useful when
working on implementing or testing a specific message type parser.
...
* 'iptn' implemented as `GeneralIptn` (Identification codes for international networks, global mobile satellite systems, IoT/M2M (country codes 881, 882, 883, 888, 991))

NOTE: For IPTN messages that cannot be automatically parsed, a manual curation workflow is available. Run `scripts/generate_iptn_template.rb` to create template files in `messages/general/iptn/manual/` that can be manually filled with structured data. The `parse_amendments.rb` script will automatically use these manual files when available.
...
$ cd scripts/
$ bundle install
$ ruby parse_amendments.rb [--type TYPE] [MESSAGE_TYPE] [FILE_PATH_PATTERN]
----

Where:

`--type`, `-t TYPE`:: Optional. The type of message to parse: 'amendment' or
'general' (default: amendment).

`MESSAGE_TYPE`:: Optional. The specific message type to parse.

** For amendment messages (default: E118_IIN), must be one of the supported
amendment types listed in `Ituob::Models::OldIssue::AMENDMENT_TYPE_TO_CLASS`.

** For general messages (default: running_annexes), must be one of the supported
general message types listed in
`Ituob::Models::OldIssue::GENERAL_TYPE_TO_CLASS`.

`FILE_PATH_PATTERN`:: Optional. A specific file or pattern to process. If not
provided, all files of the specified message type will be processed.
...
# Parse all running_annexes general messages
$ ruby parse_amendments.rb -t general

# Parse all approved_recommendations general messages
$ ruby parse_amendments.rb -t general approved_recommendations

# Parse a specific running_annexes general message file
$ ruby parse_amendments.rb -t general running_annexes 1000.yaml

# Parse all ipns general messages
$ ruby parse_amendments.rb -t general ipns

# Parse a specific ipns general message file
$ ruby parse_amendments.rb -t general ipns 993.yaml

# Parse all iptn general messages
$ ruby parse_amendments.rb -t general iptn

# Parse a specific iptn general message file
$ ruby parse_amendments.rb -t general iptn 981.yaml
...
. Look for message files in the appropriate directory:

** For amendment messages: `messages/amendments/[MESSAGE_TYPE]/`
** For general messages: `messages/general/[MESSAGE_TYPE]/`

. Parse each file using the appropriate message class

. Output the parsed content in YAML format to the console

This is particularly useful when developing and testing new message type
parsers, as it allows you to focus on a specific message type without processing
all issues.
